...
If you are doing this demo live, you can download all the files we will be viewing [here](/module4/downloads_for_demo).
...
* Explain the difference in fluoroquinolone resistance MIC between two clinical strains of *Pseudomonas aeruginosa* that appear clonal based on identical MLST ([`Pseudomonas1.fasta`, `Pseudomonas2.fasta`](/module4/sequences_for_web_demo) - these files can be found in this GitHub repo). Hint, look at SNPs.
...
Login into your course account’s working directory and make a module4 directory:
...
```bash
ls ~/CourseData/module4/ecoli
```

RGI has already been installed using Mamba, list all the available software in Mamba, activate RGI, and then review the RGI help screen:
...
We don’t have time to analyze all 39 samples, so let’s analyze 1 as an example (the course GitHub repo contains an EXCEL version of the resulting [ED010.txt](/module4/rgi_main_results/ED010.xlsx) file). When analyzing FASTA files we use the **main** sub-command, here with default settings “**Perfect and Strict hits only**”, "**Exclude nudge**", and "**High quality/coverage**":

```bash
rgi main -h
rgi main -i ~/CourseData/module4/ecoli/ED010.fasta -o ED010 -t contig -a DIAMOND -n 4 --local --clean
ls
less ED010.json
less ED010.txt
...
An additional 10 nudged Strict annotations (possible partial genes for *Escherichia coli* emrE, EF-Tu mutants conferring resistance to Pulvomycin, and AcrF) and 396 Loose annotations have been added to investigate for leads that could explain the observed phenotype. Note this scenario is unlikely for clinical isolates given CARD's reference data, but is possible for environmental isolates. The multiple putative gene fragments found via the Nudge may suggest genome assembly problems.
                
</details>

We have pre-compiled results for all 39 samples under “**Perfect and Strict hits only**"", "**Exclude nudge**", and "**High quality/coverage**", so let’s try RGI’s heat map tool ([pre-compiled images](/module4/rgi_main_results) can be downloaded or viewed from the course GitHub repo):

```bash
ls ~/CourseData/module4/ecoli_json
rgi heatmap -h
rgi heatmap -i ~/CourseData/module4/ecoli_json -o heatmap
rgi heatmap -i ~/CourseData/module4/ecoli_json -o cluster_both --cluster both
rgi heatmap -i ~/CourseData/module4/ecoli_json -o cluster_both_frequency --frequency --cluster both
ls
```
...
You can use the following annotation file to visualize all of the RGI results in the context of [Microreact](https://microreact.org) visualizations of a phylogenetic tree of these isolates: [RGI microreact results plus earlier derived whole genome SNP tree](/module4/for_microreact).
...
rgi load --card_json ./card.json --card_annotation card_database_v4.0.1.fasta --local
ls
```
...
```bash
ls ~/CourseData/module4/gut_sample
less ~/CourseData/module4/gut_sample/gut_R1.fastq
```

We can now map the metagenomic reads to the sequences in CARD's protein homolog models using the KMA algorithm:

```bash
rgi bwt -1 ~/CourseData/module4/gut_sample/gut_R1.fastq -2 ~/CourseData/module4/gut_sample/gut_R2.fastq -a kma -n 4 -o gut_sample.kma --local
ls
```
...
However, the file we are most interested in for now is [`gut_sample.kma.gene_mapping_data.txt`](/module4/rgi_bwt_results/gut_sample.kma.gene_mapping_data.xlsx) and the course GitHub repo contains an EXCEL version for easy viewing, but let's look at it on the command line:
...
We can repeat the read mapping analysis, but include more sequence variants in the reference set by including the [CARD Resistomes & Variants data set](https://card.mcmaster.ca/prevalence). First we need to acquire the Resistomes & Variant data from the CARD website:
...
rgi load --card_json ./card.json --wildcard_annotation wildcard_database_v4.0.2.fasta --wildcard_index ./wildcard/index-for-model-sequences.txt --card_annotation card_database_v4.0.1.fasta --local
```

Map reads to canonical CARD (reference sequences from the literature) **plus** predicted AMR resistance alleles and sequence variants from bulk resistome analyses, i.e. [CARD Resistomes & Variants data set](https://card.mcmaster.ca/prevalence):

> THE FOLLOWING STEPS TAKE TOO LONG, DO NOT PERFORM DURING DEMO SESSION, INSTEAD PLEASE VIEW PRE-COMPILED RESULTS. FEEL FREE TO TRY THESE STEPS OUTSIDE OF CLASS.

```bash
rgi bwt -1 ~/CourseData/module4/gut_sample/gut_R1.fastq -2 ~/CourseData/module4/gut_sample/gut_R2.fastq -a kma -n 4 -o gut_sample_wildcard.kma --local --include_wildcard
ls
```

The pre-compiled results can be viewed in the EXCEL version of [`gut_sample_wildcard.kma.gene_mapping_data.txt`](/module4/rgi_bwt_results/gut_sample_wildcard.kma.gene_mapping_data.xlsx) in the GitLab repo, but let's first compare statistics, where you'll see we aligned some additional reads:

> YOU CAN EXECUTE THESE COMMANDS AS WE HAVE PROVIDED PRE-COMPUTED RESULTS.

```bash
clear
cat ~/CourseData/module4/kmaresults/gut_sample.kma.overall_mapping_stats.txt
cat ~/CourseData/module4/kmaresults/gut_sample_wildcard.kma.overall_mapping_stats.txt
cut -f 1 ~/CourseData/module4/kmaresults/gut_sample_wildcard.kma.gene_mapping_data.txt | sort -u | wc -l
ls
```

Looking at the pre-compiled EXCEL spreadsheet, note that we have more information based on [CARD Resistomes & Variants data set](https://card.mcmaster.ca/prevalence), such as mappings to multiple alleles, flags for association with plasmids, and taxonomic distribution of the mapped alleles.
...
Chan Zuckerberg ID (CZ ID) has implemented a web-based platform for RGI analysis of metagenomic sequencing reads: CZ ID AMR Pipeline Workflow: https://chanzuckerberg.zendesk.com/hc/en-us/articles/15091031482644-AMR-Pipeline-Workflow. See the [publication](https://pubmed.ncbi.nlm.nih.gov/40329334/).

## New pathogen-of-origin prediction using k-mers!

Once you detect ARGs in metagenomic read sequences, how do you determine their pathogen-of-origin? The CARD team have released a validated k-mer tool for these predictions, see the [pre-print](https://www.biorxiv.org/content/10.1101/2025.09.15.676352v1).
