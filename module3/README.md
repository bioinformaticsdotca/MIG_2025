...
Running all 45 genomes is impractical during a short tutorial, so we examine one genome.

Questions to answer:

- How many predicted plasmids?  
- What are their sizes?  
- What is the predicted host range?  
- Which reference plasmids are closest?

### 1a: Activate the environment. 
To run MOB-suite, you first need to activate the Conda environment on the server. You do this with the following command:
**Do:**
...
### Choosing a genome (prescriptive way):
...
### Choosing a genome (fun, random way):
...
### 1c: Let’s predict some plasmids! 
Finally! OK, so we’re ready to use the MOB-suite tools. Specifically, we’re going to use mob_recon to assign contigs to either the chromosome or one of the plasmid clusters defined in the reference database.
...
Do:
https://islandcompare.ca/analysis?id=9d05ae20-0caf-11ef-8455-fd7d6262f3b3
Ignore the job name which suggests these are E. coli; they are Salmonella, I swear! They’re practically the same species anyway. 
[awkward silence]
You may need to click on the “recent jobs” tab to access the Aquifex aeolicus Salmonella genomes.
Do:
